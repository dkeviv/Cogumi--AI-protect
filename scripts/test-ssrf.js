 ...
 * 
 * Tests the validateAgentUrl function with various malicious and legitimate URLs
 */

// Import from the TypeScript source directly
const { validateAgentUrl } = require('../packages/shared/src/url-security.ts');
...
const testCases = [
  ...
  { url: 'http://metadata.azure.com/', expected: false, reason: 'Azure metadata endpoint' },
  ...
  { url: 'http://172.16.0.1/', expected: false, reason: 'Private IP 172.16.x' },
  ...
  { url: 'ftp://attacker.com/exfil', expected: false, reason: 'FTP protocol' },
  { url: 'data:text/html,<script>alert(1)</script>', expected: false, reason: 'Data protocol' },
